...
!function() {
			...
			this.viewer.hide().on("edit", this.onEditAnnotation).on("delete", this.onDeleteAnnotation).addField({
				load: function(field, annotation) {
					if (annotation.text) {
						...
						var name = document.createTextNode(annotation.time);
						nameP.appendChild(name);
						nameP.id = "nameP"
						$(field).append(pre)
						$(field).append(nameP)
					} else {
						$(field).html("<i>" + _t("Kommentti puuttuu") + "</i>")
					}
	...
	Annotator.Plugin.Store = function(_super) {
		...
		Store.prototype._onLoadAnnotations = function(data) {
			...
			let many = parseInt(Object.keys(data)[Object.keys(data).length - 1])
			console.log(many);
			annotationMap = {};
          ...
          if (annotationMap[a.id]) {
					...
					if (document.getElementById('annAll') === null) {
						console.log("ei ole");
						var el1 = document.createElement('div');
						var elOpen = document.createElement('div');
						var c1 = document.createElement('div');
						var infoNode = document.createTextNode('Kommentit tällä sivulla: ');
						var newContent = document.createTextNode(a.time + " - " + a.text + " @ " + (a.quote).slice(0, 35));
						c1.appendChild(newContent);
						el1.appendChild(infoNode);
						el1.appendChild(c1);
						el1.id = 'annAll';
						elOpen.id = 'annAllOpen';
						var xtext = document.createTextNode('Show all');
						elOpen.appendChild(xtext);
						function showAlert() {
							$("#annAll").toggle();
						}
						;elOpen.onclick = showAlert;
						elOpen.appendChild(el1);
						document.body.appendChild(elOpen);
					} else {
						var el1 = document.getElementById('annAll');
						var c1 = document.createElement('div');
						var newContent = document.createTextNode(a.time + "- " + a.text + " @ " + (a.quote).slice(0, 35));
						c1.appendChild(newContent);
						el1.appendChild(c1);
          ...
          console.log("nullero kohdassa", _l);
        }
			}
		...
		Store.prototype._urlFor = function(action, id) {
			var url;
			url = this.options.prefix != null ? this.options.prefix : "/4";
			console.log('Annotations: ', this.annotations.length);
			url += this.options.urls[action];
			url = url.replace(/\/:id/, id != null ? "/" + id : "/" + ( this.annotations.length > 1 ? this.annotations[this.annotations.length-2].id+1 : 0) + "");
			url = url.replace(/:id/, id != null ? id : "/" + ( this.annotations.length > 1 ? this.annotations[this.annotations.length-2].id+1 : 0) + "");
			return url
		}
		;
		...
		Store.prototype._dataFor = function(annotation) {
			...
			$.extend(annotation, {
				'id': ( this.annotations.length > 1 ? this.annotations[this.annotations.length-2].id+1 : 0)
			},{
